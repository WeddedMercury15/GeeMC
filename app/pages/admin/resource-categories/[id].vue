...
const { data } = await useFetch<{ templates: ResourceTemplate[], categories: ResourceCategory[] }>('/api/admin/resource-categories/page')
...
const category = computed(() => {
  ...
  return (data.value?.categories ?? []).find(c => c.id === categoryId.value) ?? null
})
...
const defaultExpandedParentTreeKeys = computed(() => {
  ...
  walk(parentCategoryItems.value)
  return keys
})
const parentDisplayLabel = computed(() => {
  if (!form.parentCategoryId) return t('admin.resource_categories_page.fields.parent_none')
  const picked = parentCategoryItems.value.find(item => item.value === form.parentCategoryId)
  if (picked) return picked.label
  const fromCategory = categories.value.find(item => item.id === form.parentCategoryId)
  return fromCategory?.name ?? t('admin.resource_categories_page.fields.parent_none')
})
...
watch([templates, category], () => {
  if (!category.value) return
  form.name = category.value.name
  form.slug = category.value.slug
  form.description = category.value.description ?? ''
  form.icon = category.value.icon || 'i-lucide-folder'
  form.templateId = category.value.templateId
  form.allowLocal = category.value.allowLocal
  form.allowExternal = category.value.allowExternal
  form.allowCommercialExternal = category.value.allowCommercialExternal
  form.allowFileless = category.value.allowFileless
  form.alwaysModerateCreate = category.value.alwaysModerateCreate
  form.alwaysModerateUpdate = category.value.alwaysModerateUpdate
  form.enableVersioning = category.value.enableVersioning
  form.enableSupportUrl = category.value.enableSupportUrl
  form.requirePrefix = category.value.requirePrefix
  form.minTags = category.value.minTags
  form.parentCategoryId = category.value.parentCategoryId
  form.displayOrder = category.value.displayOrder
}, { immediate: true })
...
async function submit() {
  ...
  if (form.templateId === undefined || !category.value) return
  try {
...
<template>
  <div class="space-y-6">
    ...
    <UEmpty
      ...
      :description="t('admin.resource_categories_page.not_found')"
    />
