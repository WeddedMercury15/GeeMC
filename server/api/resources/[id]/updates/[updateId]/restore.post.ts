...
import { resourceCategories, resourceUpdates, resources } from '../../../../../database/schema'
...
export default defineEventHandler(async (event) => {
  ...
  const [existing] = await db
    .select({ id: resourceUpdates.id, messageState: resourceUpdates.messageState, isDescription: resourceUpdates.isDescription })
    .from(resourceUpdates)
  ...
  if (existing.messageState !== 'deleted') {
    return { success: true }
  }

  await db
    .update(resourceUpdates)
    .set({ messageState: 'visible' })
    .where(and(eq(resourceUpdates.id, updateId), eq(resourceUpdates.resourceId, resourceId)))
