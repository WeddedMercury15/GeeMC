  ...
  <div class="space-y-6">
    <div>
      <h2 class="text-3xl font-bold tracking-tight">
        {{ t('admin.resource_followers_page.title') }}
      </h2>
      <p class="text-(--ui-text-muted)">
        {{ t('admin.resource_followers_page.subtitle') }}
      </p>
    </div>

    <UCard>
      <div class="text-sm text-(--ui-text-muted)">
        {{ t('admin.resource_followers_page.total', { total: totalFollows }) }}
      </div>
    </UCard>

    <UCard>
      <div class="text-lg font-semibold mb-3">
        {{ t('admin.resource_followers_page.top_resources') }}
      </div>
      <div
        v-if="pending"
        class="text-sm text-(--ui-text-muted)"
      >
        {{ t('admin.resource_followers_page.loading') }}
      </div>
      <div
        v-else-if="topResources.length === 0"
        class="text-sm text-(--ui-text-muted)"
      >
        {{ t('admin.resource_followers_page.empty') }}
      </div>
      <div
        v-else
        class="space-y-2"
      >
        <div
          v-for="row in topResources"
          :key="row.resourceId"
          class="flex items-center justify-between border border-(--ui-border) rounded-lg p-3"
        >
          <NuxtLink
    ...
    <UCard>
      <div class="text-lg font-semibold mb-3">
        {{ t('admin.resource_followers_page.recent_follows') }}
      </div>
      <div
        v-if="pending"
        class="text-sm text-(--ui-text-muted)"
      ...
      <div
        v-else-if="recentFollows.length === 0"
        class="text-sm text-(--ui-text-muted)"
      >
        {{ t('admin.resource_followers_page.empty') }}
      </div>
      ...
      >
        <div
          v-for="row in recentFollows"
          :key="`${row.userId}-${row.resourceId}-${row.createdAt}`"
